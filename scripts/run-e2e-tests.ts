...
class E2ETestRunner {
	...
	async checkFoundryServer(): Promise<boolean> {
		try {
			const { stdout } = await execAsync(
				`curl -s -o /dev/null -w "%{http_code}" ${this.foundryUrl}`,
			);
			const statusCode = Number.parseInt(stdout.trim());
	 ...
	 */
	async waitForFoundryServer(timeout = 60000): Promise<boolean> {
...
async function main() {
	...
	try {
		...
		if (!skipServerCheck) {
			...
			if (!serverReady) {
				console.error(
					"Please ensure FoundryVTT server is running with: fvtt launch",
				);
				process.exit(1);
			}
		}
