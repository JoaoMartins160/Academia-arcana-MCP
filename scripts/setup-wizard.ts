...
class SetupWizard {
  ...
  async run(): Promise<void> {
    ...
    console.log(
      "This wizard will help you configure your MCP server connection to FoundryVTT.\n",
    );
    ...
    try {
      ...
      if (!isReachable) {
        console.log(
          "⚠️ Cannot reach FoundryVTT. Please ensure it's running and try again.",
        );
        process.exit(1);
      }
  ...
  private async detectFoundryVTT(): Promise<void> {
    ...
    console.log(
      "2. Remote FoundryVTT (reverse proxy, cloud hosting, or remote server)",
    );
    ...
    for (const url of commonUrls) {
      if (await this.testUrl(url)) {
        const useDetected = await this.question(
          `✅ Found FoundryVTT at ${url}. Use this URL? (y/n): `,
        );
  ...
  private async configureRemoteFoundryVTT(): Promise<void> {
    ...
    console.log(
      "For remote setups, you'll need the full URL including protocol.",
    );
    ...
    const remoteUrl = await this.question(
      "🌐 Enter your remote FoundryVTT URL: ",
    );
    ...
    if (
      ...
      console.log(
        "⚠️  Warning: This appears to be a local URL. Make sure this is correct for your setup.",
      );
    }
  ...
  private async testConnectivity(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        `⚠️ Received status ${response.status} - server may not be FoundryVTT\n`,
      );
      return false;
    } catch (error) {
      console.log(
  ...
  private async chooseAuthMethod(): Promise<void> {
    ...
    console.log(
      "1. API Key (Recommended - requires Foundry Local REST API module)",
    );
  ...
  private async configureAuthentication(): Promise<void> {
    ...
    if (this.config.authMethod === "api-key") {
      ...
      console.log(
        '1. Install the "Foundry Local REST API" module in FoundryVTT',
      );
      console.log("2. Enable the module in your world settings");
      console.log(
        "3. Go to Settings → Module Settings → Foundry Local REST API",
      );
    ...
    try {
      if (this.config.authMethod === "api-key" && this.config.apiKey) {
        ...
        const response = await axios.get(
          `${this.config.foundryUrl}/api/status`,
          {
            headers: {
        ...
        if (response.status === 200) {
          ...
          console.log(
            `📊 Connected to world: ${response.data.world || "Unknown"}\n`,
          );
        }
        ...
        console.log(
          "✅ Credentials configured (full test will occur at runtime)\n",
        );
      }
    } catch (error) {
      if (axios.isAxiosError(error)) {
        ...
        if (error.response?.status === 404) {
          throw new Error(
            "REST API endpoints not found - ensure the module is installed and enabled",
          );
        }
      }
      throw new Error(
  ...
  private async runFinalValidation(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `⚠️ Validation test encountered issues: ${error instanceof Error ? error.message : error}`,
      );
      console.log(
        "The configuration has been saved, but you may need to troubleshoot connectivity.\n",
      );
    }
  }
