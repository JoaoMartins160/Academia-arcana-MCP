...
function validateFilesEntries() {
	...
	for (const entry of entries) {
		...
		if (!existsSync(rootDir)) {
			fail(
				`package.json "files" entry "${entry}" — root dir does not exist: ${rootDir}`,
			);
		}
		const re = globToRegExp(cleaned);
		const matches = walk(rootDir).some((abs) =>
			re.test(relative(repoRoot, abs)),
		);
		if (!matches) {
	...
	console.log(
		`✅ All ${checked.length} "files" entries reference existing paths or globs`,
	);
}
...
function packTarball() {
	...
	try {
		...
		tarballName = Array.isArray(packInfo)
			? packInfo[0]?.filename
			: packInfo?.filename;
	} catch (err) {
		fail(
...
function installInConsumer(tarball) {
	...
	if (install.status !== 0) {
		fail(
			`npm install <tarball> failed (exit ${install.status})`,
			install.stderr,
		);
	}
...
function runInstalledServer({ consumer, serverPath }) {
	return new Promise((resolve) => {
		...
		const timeout = setTimeout(() => {
			...
			finish(
				false,
				`timed out after ${STARTUP_TIMEOUT_MS}ms waiting for banner`,
			);
		}, STARTUP_TIMEOUT_MS);
	});
}
