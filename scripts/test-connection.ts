...
async function testConnection() {
	...
	try {
		...
		console.log(
			`   REST Module: ${config.foundry.useRestModule ? "✅" : "❌"}`,
		);
		console.log(
			`   API Key: ${config.foundry.apiKey ? "✅ Configured" : "❌ Not set"}`,
		);
		console.log(
			`   Username: ${config.foundry.username ? "✅ Configured" : "❌ Not set"}\n`,
		);
		...
		try {
			...
			console.log(
				`❌ Dice rolling failed: ${error instanceof Error ? error.message : error}\n`,
			);
		}
		...
		try {
			...
			console.log(
				`⚠️  Actor search limited: ${error instanceof Error ? error.message : error}\n`,
			);
		}
		...
		try {
			...
			console.log(
				`⚠️  Scene info limited: ${error instanceof Error ? error.message : error}\n`,
			);
		}
		...
		try {
			...
			console.log(
				`⚠️  WebSocket connection issues: ${error instanceof Error ? error.message : error}\n`,
			);
		}
		...
		console.log(
			`   - Data access: ${config.foundry.useRestModule ? "✅ Full" : "⚠️  Limited"}`,
		);
		console.log(
			`   - WebSocket: ${config.foundry.useRestModule ? "✅" : "⚠️  Basic"}`,
		);
		...
		try {
			...
			if (isLocal) {
				...
				console.log(
					"   3. Try accessing FoundryVTT directly: http://localhost:30000",
				);
				...
				console.log(
					"   3. Verify reverse proxy is forwarding to FoundryVTT correctly",
				);
				console.log(
					"   4. Test WebSocket upgrades are working (required for FoundryVTT)",
				);
				...
				console.log(
					"   3. Verify the port is open and not blocked by firewall",
				);
