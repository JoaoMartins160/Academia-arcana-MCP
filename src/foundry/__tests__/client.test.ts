...
describe("FoundryClient", () => {
  ...
  describe("constructor", () => {
    ...
    it("should throw on empty baseUrl", () => {
      expect(() => new FoundryClient({ baseUrl: "" })).toThrow(
        "baseUrl is required",
      );
    });

    it("should throw on invalid baseUrl", () => {
      expect(() => new FoundryClient({ baseUrl: "not-a-url" })).toThrow(
        "Invalid baseUrl",
      );
    });
  ...
  describe("REST API mode (with apiKey)", () => {
    ...
    it("should search actors via REST API", async () => {
      const mockData = {
        actors: [{ _id: "1", name: "Hero", type: "character" }],
      };
  ...
  describe("retry/backoff matrix (CN-6)", () => {
    ...
    beforeEach(() => {
      ...
      mockAxios.isAxiosError = ((
        ...
        (e as { isAxiosError?: boolean }).isAxiosError ===
          true) as typeof axios.isAxiosError;
    });
    ...
    it("does NOT retry on 400 Bad Request", async () => {
      ...
      await expect(client.searchActors({ query: "x" })).rejects.toThrow(
        "HTTP 400",
      );
  ...
  describe("dice rolling", () => {
    ...
    it("should validate dice formula", async () => {
      ...
      await expect(client.rollDice("DROP TABLE")).rejects.toThrow(
  ...
  describe("refreshWorldData listener cleanup", () => {
    ...
    it("removes the world listener on the timeout path", async () => {
      ...
      await expect(client.refreshWorldData()).rejects.toThrow(
        "Refresh timeout",
      );
