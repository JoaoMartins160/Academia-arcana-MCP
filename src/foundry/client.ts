	...
	private fallbackDiceRoll(
		...
		while ((match = tokenRegex.exec(cleanFormula)) !== null) {
			...
			if (token.includes("d")) {
				...
				const numDice = Number.parseInt(numDiceStr || "1", 10);
				const numSides = Number.parseInt(numSidesStr || "6", 10);
				const results = [];
				let termTotal = 0;
				for (let i = 0; i < numDice; i++) {
					const r = Math.floor(Math.random() * numSides) + 1;
					results.push({ result: r, active: true });
					termTotal += r;
				}
				total += operator === "-" ? -termTotal : termTotal;
				breakdown.push(
					`${operator === "-" ? "-" : ""}${numDice}d${numSides} (${results.map((r) => r.result).join(", ")})`,
				);
				terms.push({
					class: "Die",
					options: {},
