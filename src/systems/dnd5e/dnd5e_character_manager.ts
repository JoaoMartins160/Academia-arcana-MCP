...
export class Dnd5eCharacterManager extends EventEmitter {
  ...
  async levelUpCharacter(
    ...
    // Calculate HP gain
    const hitPointsGained = this.calculateHitPointGain(
      actor,
      options.hitPointMethod || "average",
    );
    ...
    const newSpells =
      options.newSpells || this.determineNewSpells(actor, newLevel);
    const newFeatures =
      options.newFeatures || this.determineNewFeatures(actor, newLevel);
    const newSkills =
      options.newSkills || this.determineNewSkills(actor, newLevel);
  ...
  async calculateExperienceRequired(
    ...
    const currentXP =
      xpTable[Math.min(currentLevel - 1, xpTable.length - 1)] || 0;
    const target = targetLevel || currentLevel + 1;
    const requiredXP =
      xpTable[Math.min(target - 1, xpTable.length - 1)] ||
      xpTable[xpTable.length - 1];
    const nextLevelXP =
      xpTable[Math.min(currentLevel, xpTable.length - 1)] ||
      xpTable[xpTable.length - 1];
  ...
  async transferItem(
    ...
    logger.info(
      `Transferred item ${itemId} from ${fromActorId} to ${toActorId}`,
    );
    return transaction;
  }
  ...
  async restoreResources(
    ...
    for (const resource of resources) {
      if (
        ...
        (restType === "long_rest" &&
          (resource.resetType === "long_rest" ||
            resource.resetType === "short_rest"))
      ) {
  ...
  private calculateHitPointGain(
    _actor: FoundryActor,
    method: "roll" | "average" | "max",
  ): number {
