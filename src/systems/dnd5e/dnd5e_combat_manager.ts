...
export class Dnd5eCombatManager extends EventEmitter {
  ...
  getCombatHistory(limit = 50): CombatEvent[] {
    return this.combatHistory.slice(-limit);
  }

  async startCombat(
    combatants: Partial<CombatantState>[],
  ): Promise<CombatState> {
  ...
  async rollInitiative(
    ...
    for (const combatant of combatants) {
      ...
      logger.debug(
        `${combatant.name} rolled initiative: ${total} (${roll} + ${modifier})`,
      );
    }

    // Re-sort combatants by initiative
    this.currentCombat.combatants.sort(
  ...
  async applyDamage(
    ...
    const combatant = this.currentCombat.combatants.find(
      (c) => c.id === combatantId,
    );
    if (!combatant) {
  ...
  private handleCombatStart(event: CombatEvent): void {
    logger.info(
      `Combat started with ${event.combat.combatants.length} combatants`,
    );
  }
  ...
  private handleRoundEnd(event: CombatEvent): void {
    logger.info(
      `Round ${event.combat.round - 1} ended, starting Round ${event.combat.round}`,
    );
  }
  ...
  getCombatStatistics(): {
    ...
    const stats = {
      ...
      totalTurns:
        this.currentCombat.round * this.currentCombat.combatants.length +
        this.currentCombat.turn,
      ...
      activeEffects: this.currentCombat.combatants.reduce(
  ...
  private calculateAverageTurnTime(): number {
    const turnEvents = this.combatHistory.filter(
      (e) => e.type === "turn_start",
    );
    ...
    for (let i = 1; i < turnEvents.length; i++) {
      ...
      if (currentEvent && previousEvent) {
        totalTime +=
          currentEvent.timestamp.getTime() - previousEvent.timestamp.getTime();
      }
    }
  ...
  suggestTacticalActions(combatantId: string): string[] {
    ...
    if (combatant.conditions.includes("grappled")) {
      suggestions.push(
        "Attempt to escape grapple with Athletics or Acrobatics",
      );
    }
    ...
    if (!combatant.actions.action) {
      suggestions.push(
        "Use your Action for Attack, Cast a Spell, or other actions",
      );
    }
