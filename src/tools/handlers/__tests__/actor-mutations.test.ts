...
import { handleUpdateActorAttribute } from "../actor/actor-mutations.js";
	...
	describe("handleUpdateActorAttribute", () => {
		...
		it("round-trips a resource/currency value", async () => {
			...
			const result = await handleUpdateActorAttribute(
				{
					actorId: VALID_ID,
					patch: { "currency.gp": 12, "resources.primary.value": 3 },
				},
				mockClient,
			);
		...
		const buildClient = (opts: {
			...
			if (opts.actor) {
				vi.spyOn(client, "getActor").mockResolvedValue(
					opts.actor as unknown as Awaited<
						ReturnType<FoundryClient["getActor"]>
					>,
				);
			}
