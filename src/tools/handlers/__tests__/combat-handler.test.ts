...
import { handleGetCombatState } from "../combat/combat-handler.js";
...
describe("handleGetCombatState", () => {
  ...
  it("appends HP/AC from raw actor when actorId resolves", async () => {
    const combat = buildCombat({
      combatants: [
        buildCombatant({ name: "Orc", initiative: 12, actorId: "actor-99" }),
      ],
    });
  ...
  it("omits HP/AC when raw actor lookup returns undefined", async () => {
    const combat = buildCombat({
      combatants: [
        buildCombatant({ name: "Ghost", initiative: 7, actorId: "missing-id" }),
      ],
    });
    const client = {
      getCombatState: vi.fn().mockReturnValue(combat),
