...
import {
  ...
  handleSetInitiative,
  handleStartCombat,
} from "../combat/combat-mutations.js";
...
describe("computeNextTurn", () => {
  ...
  describe("skipDefeated", () => {
    ...
    const makeTrio = (
      turn: number | null,
      round: number,
      defeated: [boolean, boolean, boolean],
    ) =>
      makeCombat({
        turn,
        round,
...
describe("Combat mutation handlers", () => {
  ...
  describe("handleNextTurn", () => {
    it("advances the active combat and reports the new round/turn/combatant", async () => {
      const client = createMockClient({
        combat: makeCombat({ turn: 0, round: 1 }),
      });
      ...
      expect(client.updateCombat).toHaveBeenCalledWith(COMBAT_ID, {
        turn: 1,
        round: 1,
      });
    });
    ...
    it("propagates the write-disabled guard error", async () => {
      ...
      await expect(handleNextTurn({}, client)).rejects.toThrow(
  ...
  describe("handleStartCombat", () => {
    ...
    it("seeds combatants from explicit tokenIds, resolving actorId from the token", async () => {
      ...
      const client = createMockClient({
        scenes: [
          makeScene(true, [{ _id: TOKEN_A, actorId: "actoraaaaaaaaaaa" }]),
        ],
        startCombat,
      });
    ...
    it("defaults to every token on the active scene when tokenIds is omitted", async () => {
      ...
      const client = createMockClient({
        scenes: [
          makeScene(true, [
            { _id: TOKEN_A, actorId: "actoraaaaaaaaaaa" },
            { _id: TOKEN_B },
          ]),
        ],
        startCombat,
    ...
    it("honours an explicit sceneId", async () => {
      ...
      const client = createMockClient({
        scenes: [
          makeScene(false, []),
          { _id: "othersssssssssss", name: "Other", active: true },
        ],
        startCombat,
      });
    ...
    it("raises McpError when a requested token is not on the scene", async () => {
      const client = createMockClient({
        scenes: [makeScene(true, [{ _id: TOKEN_A }])],
      });
      await expect(
        handleStartCombat({ tokenIds: [TOKEN_B] }, client),
  ...
  describe("handleSetInitiative", () => {
    it("sets initiative on the active combat by default", async () => {
      ...
      expect(client.setCombatantInitiative).toHaveBeenCalledWith(
        COMBAT_ID,
        COMBATANT_ID,
        18,
      );
    ...
    it("rejects a missing combatantId before reaching the client", async () => {
      const client = createMockClient({ combat: makeCombat() });
      await expect(
        handleSetInitiative({ combatantId: "", initiative: 5 }, client),
      ).rejects.toThrow(McpError);
    ...
    it("rejects a non-finite initiative before reaching the client", async () => {
      ...
      await expect(
        handleSetInitiative(
          { combatantId: COMBATANT_ID, initiative: Number.NaN },
          client,
        ),
      ).rejects.toThrow(McpError);
      expect(client.setCombatantInitiative).not.toHaveBeenCalled();
    ...
    it("raises McpError when no active combat and no combatId", async () => {
      ...
      await expect(
        handleSetInitiative(
          { combatantId: COMBATANT_ID, initiative: 5 },
          client,
        ),
      ).rejects.toThrow(McpError);
      expect(client.setCombatantInitiative).not.toHaveBeenCalled();
...
describe("FoundryClient combat mutations", () => {
    ...
    if (opts.connected !== false) {
      (
        client as unknown as {
          socket: { connected: boolean; emit: SocketEmitMock };
        }
      ).socket = {
        connected: true,
        emit,
  ...
  it("updateCombat emits a Combat update with turn/round and no parentUuid", async () => {
    ...
    const [event, body] = emit.mock.calls[0] as unknown[] as [
      string,
    ...
    expect(
      (body.operation as Record<string, unknown>).parentUuid,
    ).toBeUndefined();
  });
  ...
  it("endCombat emits a Combat delete carrying ids", async () => {
    ...
    const [, body] = emit.mock.calls[0] as unknown[] as [
      string,
      Record<string, unknown>,
    ];
    expect(body).toMatchObject({
  ...
  it("startCombat creates a Combat then embedded Combatants with parentUuid", async () => {
    ...
    const [, combatBody] = emit.mock.calls[0] as unknown[] as [
      string,
      Record<string, unknown>,
    ];
    ...
    expect(
      (combatBody.operation as Record<string, unknown>).parentUuid,
    ).toBeUndefined();

    const [, combatantBody] = emit.mock.calls[1] as unknown[] as [
      string,
      Record<string, unknown>,
    ];
  ...
  it("startCombat rejects a malformed scene id", async () => {
    ...
    await expect(client.startCombat("short", [])).rejects.toThrow(
      /Invalid sceneId/,
    );
  });
  ...
  it("startCombat rejects a malformed token id", async () => {
    const { client } = buildClient({});
    await expect(
      client.startCombat("sceneididididid1", [
        { tokenId: "short", sceneId: "sceneididididid1" },
      ]),
  ...
  it("rejects writes when the Socket.IO connection is not active", async () => {
    ...
    await expect(client.endCombat(COMBAT_ID)).rejects.toThrow(
  ...
  it("rejects a malformed combat id", async () => {
    const { client } = buildClient({});
    await expect(client.updateCombat("short", { turn: 1 })).rejects.toThrow(
      /Invalid combatId/,
    );
  });
  ...
  it("rejects a malformed combatant id", async () => {
    const { client } = buildClient({});
    await expect(
      client.setCombatantInitiative(COMBAT_ID, "short", 5),
    ).rejects.toThrow(/Invalid combatantId/);
  });

  it("rejects a non-finite initiative at the client", async () => {
    const { client } = buildClient({});
    await expect(
      client.setCombatantInitiative(
        COMBAT_ID,
        COMBATANT_ID,
