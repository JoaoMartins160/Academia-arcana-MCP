...
import { handleSearchCompendium } from "../compendium/compendium-handler.js";
...
describe("Compendium handlers", () => {
  ...
  const createMockClient = (
    result: CompendiumSearchResult = divineSmite2014,
  ): FoundryClient => {
    return {
  ...
  describe("handleSearchCompendium", () => {
    it("returns compendium entries with disambiguating metadata", async () => {
      ...
      const result = await handleSearchCompendium(
        { query: "Divine Smite", limit: 20 },
        mockClient,
      );
    ...
    it("passes filters through to the client", async () => {
      ...
      await handleSearchCompendium(
        {
          query: "Divine Smite",
          filters: { compendiumId: "dnd5e.spells", spellLevel: 1 },
        },
        mockClient,
      );
    ...
    it("rejects with InvalidParams on missing query", async () => {
      const mockClient = createMockClient();
      await expect(
        handleSearchCompendium({ query: "" }, mockClient),
    ...
    it("propagates client errors through withToolError", async () => {
      ...
      await expect(
        handleSearchCompendium({ query: "Divine Smite" }, mockClient),
      ).rejects.toThrow();
    });
  });
});
