...
import { handleGetRecentLogs } from "../diagnostic/diagnostic-handler.js";
...
function extractLines(
  result: Awaited<ReturnType<typeof handleGetRecentLogs>>,
): string[] {
  const text =
  ...
  return body === "No log entries found."
    ? []
    : body.split("\n").filter(Boolean);
}
...
describe("handleGetRecentLogs", () => {
  ...
  describe("level filter", () => {
    ...
    it("does not throw when level is unrecognized", async () => {
      const client = mockClient(SAMPLE_LOGS);
      await expect(
        handleGetRecentLogs({ level: "INVALID_LEVEL" }, client),
  ...
  describe("since filter", () => {
    ...
    it("does not throw when since is unparseable", async () => {
      const client = mockClient(SAMPLE_LOGS);
      await expect(
        handleGetRecentLogs({ since: "!!invalid!!" }, client),
      ).resolves.toBeDefined();
    });
  });
  ...
  describe("combined filters", () => {
    ...
    it("applies since and level together", async () => {
      ...
      const client = mockClient(SAMPLE_LOGS);
      const result = await handleGetRecentLogs(
        { level: "error", since: sinceTimestamp },
        client,
      );
      const lines = extractLines(result);
      expect(lines.length).toBeGreaterThan(0);
