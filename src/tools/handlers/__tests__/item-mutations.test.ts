...
const createMockClient = (
  overrides: Partial<FoundryClient> = {},
): FoundryClient => {
  return {
...
describe("item mutation handlers", () => {
    ...
    it("rejects a missing source with InvalidParams", async () => {
      ...
      await expect(
        handleCreateActorItem(
          {
            actorId: VALID_ACTOR_ID,
            source: undefined as unknown as {
              type: "inline";
    ...
    it("rejects an unknown source type with InvalidParams", async () => {
      ...
      await expect(
        handleCreateActorItem(
          {
            actorId: VALID_ACTOR_ID,
            source: { type: "bogus" } as unknown as {
              type: "inline";
              item: Partial<FoundryItem>;
            },
          },
  ...
  describe("handleUpdateActorItem", () => {
    it("updates an item with a system patch", async () => {
      ...
      expect(mockClient.updateActorItem).toHaveBeenCalledWith(
        VALID_ACTOR_ID,
        VALID_ITEM_ID,
        {
          equipped: true,
  ...
  describe("handleDeleteActorItem", () => {
    it("deletes an item", async () => {
      ...
      expect(mockClient.deleteActorItem).toHaveBeenCalledWith(
        VALID_ACTOR_ID,
        VALID_ITEM_ID,
      );
    });
...
describe("FoundryClient item mutations (modifyDocument)", () => {
  ...
  const buildClient = (
    opts: { writeEnabled?: boolean; connected?: boolean } = {},
  ) => {
  ...
  const lastRequest = (client: FoundryClient) => {
    const emitMock = (
      client as unknown as { socket: { emit: ReturnType<typeof vi.fn> } }
    ).socket.emit;
  ...
  it("rejects writes when FOUNDRY_WRITE_ENABLED is false", async () => {
    ...
    await expect(
      client.deleteActorItem(VALID_ACTOR_ID, VALID_ITEM_ID),
    ).rejects.toThrow(/FOUNDRY_WRITE_ENABLED/);
  });
  ...
  it("rejects a malformed itemId before emitting", async () => {
    const client = buildClient();
    await expect(
      client.updateActorItem(VALID_ACTOR_ID, "short", {}),
