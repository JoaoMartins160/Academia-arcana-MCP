...
describe("Token mutation handlers", () => {
	const createMockClient = (opts: {
		...
		({
			...
			createActorStatusEffect: vi.fn(
				opts.createEffect ??
					(() => ({ _id: EFFECT_ID, name: "Prone", statuses: ["prone"] })),
			),
	...
	describe("handleMoveToken", () => {
		it("moves a located token and reports the new position", async () => {
			...
			const result = await handleMoveToken(
				{ tokenId: TOKEN_ID, x: 100, y: 200 },
				client,
			);
			...
			expect(client.moveToken).toHaveBeenCalledWith(
				SCENE_ID,
				TOKEN_ID,
				100,
				200,
		...
		it("raises McpError when the token is not found", async () => {
			...
			await expect(
				handleMoveToken({ tokenId: TOKEN_ID, x: 1, y: 2 }, client),
		...
		it("rejects a missing tokenId", async () => {
			const client = createMockClient({});
			await expect(
				handleMoveToken({ tokenId: "", x: 1, y: 2 }, client),
			).rejects.toThrow(McpError);
		});
	});
	...
	describe("handleApplyStatusEffect", () => {
		...
		it("targets the synthetic actor UUID for an unlinked token", async () => {
			...
			await handleApplyStatusEffect(
				{ tokenId: TOKEN_ID, statusId: "stunned" },
				client,
			);
...
describe("FoundryClient token mutations", () => {
	...
	it("createActorStatusEffect accepts the unlinked synthetic-actor UUID form", async () => {
		...
		await client.createActorStatusEffect(uuid, "stunned", {
			name: "Stunned",
			img: "x.png",
		});
		const [, body] = emit.mock.calls[0] as unknown[] as [
	...
	it("rejects a malformed sceneId", async () => {
		...
		await expect(client.moveToken("short", TOKEN_ID, 1, 2)).rejects.toThrow(
			/Invalid sceneId/,
		);
	});
	...
	it("rejects a malformed tokenId", async () => {
		const { client } = buildClient({});
		await expect(client.moveToken(SCENE_ID, "short", 1, 2)).rejects.toThrow(
			/Invalid tokenId/,
		);
	});
	...
	it("rejects non-finite coordinates at the client", async () => {
		const { client } = buildClient({});
		await expect(
			client.moveToken(SCENE_ID, TOKEN_ID, Number.POSITIVE_INFINITY, 0),
	...
	it("rejects a malformed actor UUID for status effects", async () => {
		const { client } = buildClient({});
		await expect(
			client.createActorStatusEffect("Actor.short", "prone"),
		).rejects.toThrow(/Invalid actor UUID/);
	});

	it("rejects a malformed effectId on delete", async () => {
		const { client } = buildClient({});
		await expect(
			client.deleteActorEffect(`Actor.${ACTOR_ID}`, "short"),
		).rejects.toThrow(/Invalid effectId/);
	});
});
