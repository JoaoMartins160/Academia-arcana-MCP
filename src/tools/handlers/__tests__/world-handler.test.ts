...
describe("handleSearchWorld", () => {
  ...
  it('returns a "no results" message when all collections are empty', async () => {
    const client = {
      searchWorld: vi
        .fn()
        .mockReturnValue({ actors: [], items: [], scenes: [], journals: [] }),
    } as unknown as FoundryClient;
  ...
  it("wraps client errors in McpError", async () => {
    ...
    await expect(handleSearchWorld({ query: "a" }, client)).rejects.toThrow(
      McpError,
    );
  });
});
