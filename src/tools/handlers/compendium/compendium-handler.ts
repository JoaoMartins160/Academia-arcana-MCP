...
/**
 * Handles get compendiums list requests.
 */
export async function handleGetCompendiumsList(
	args: {},
	foundryClient: FoundryClient,
) {
	return withToolError("get compendiums list", async () => {
		const packs = await foundryClient.getCompendiumsList();
		...
		if (!packs || packs.length === 0) {
			return {
				content: [
					{
						type: "text",
						text: "📚 **Compendium Packs**\nNo compendium packs found. Make sure the Foundry MCP Companion module is enabled.",
					},
				],
			};
		}
		...
		const packList = packs
			.map(
				(p) =>
					`- **${p.title}** (\`${p.id}\`) — Type: ${p.type} | Package: ${p.package}`,
			)
			.join("\n");
		...
		return {
			content: [
				{
					type: "text",
					text: `📚 **Compendium Packs (${packs.length})**\n\n${packList}`,
				},
			],
		};
	});
...
**Page:** ${result.page} | **Limit:** ${result.limit}${cursorNote}`,
				},
			],
		};
	});
