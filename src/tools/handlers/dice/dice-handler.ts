...
export async function handleRollDaggerheart(
	args: { modifier: number; reason?: string },
	foundryClient: FoundryClient,
): Promise<ToolResult> {
	const { modifier, reason } = args;

	logger.info(
		`Rolling Daggerheart dice with modifier: ${modifier}${reason ? ` (${reason})` : ""}`,
	);
	const result = await foundryClient.rollDaggerheart(modifier, reason);

	return {
		content: [
			{
				type: "text",
				text: `🎲 **Daggerheart Roll Result**\n**Formula:** ${result.formula}\n**Total:** ${result.total}\n**Breakdown:** ${result.breakdown}\n${result.reason ? `**Reason:** ${result.reason}\n` : ""}**Timestamp:** ${result.timestamp}\n**Debug:** ${(result as any).serverDebug}`,
			},
		],
	};
}
