...
describe("handleManageDaggerheartDomainCards", () => {
  it("should add a Domain Card item to a character actor", async () => {
    const mockClient = {
      getRawActor: vi
        .fn()
        .mockReturnValue({ name: "Valerius", type: "character" }),
      createActorItem: vi
        .fn()
        .mockResolvedValue({ _id: "card123", name: "Arcane Sense" }),
